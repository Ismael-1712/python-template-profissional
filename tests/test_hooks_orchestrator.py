...
class TestGenerateHookScript:
    ...
    def test_generate_hook_script_post_merge(self) -> None:
        ...
        assert "cortex map --output .cortex/context.json" in script
        assert "command -v cortex" in script  # Should check for cortex availability
    ...
    def test_generate_hook_script_includes_comments(self) -> None:
        ...
        # Assert
        assert "Auto-generated by CORTEX" in script or "CORTEX" in script
        assert "#" in script  # Should have comments

    def test_generate_hook_script_graceful_failure(self) -> None:
        """generate_hook_script() should allow graceful failure if command missing."""
        # Arrange
        orchestrator = HooksOrchestrator(project_root=Path("/tmp"))
        ...
        # Assert
        # Should check for command and exit 0 (gracefully) if not found
        assert "exit 0" in script
        assert "command -v" in script or "which" in script
